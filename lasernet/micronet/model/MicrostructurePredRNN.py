...
class MicrostructurePredRNN(nn.Module):
    """
    PredRNN-based model for microstructure field prediction conditioned on temperature.
    ...
    Architecture:
        ...
        PredRNN: Spatiotemporal modeling with ST-LSTM layers
        Future encoder: Processes next temperature frame (1 channel → 64 features)
        Fusion: Concatenate PredRNN output + future temp features
    ...
    def __init__(
        ...
        output_channels: int = 9,   # 9 microstructure (IPF only)
        hidden_channels: List[int] = [16, 32, 64],
        rnn_hidden: int = 64,
        rnn_layers: int = 4,  # PredRNN typically uses 4 layers
        ...
        # ==================== PREDRNN ====================
        # Spatiotemporal modeling with ST-LSTM layers
        self.pred_rnn = PredRNN(
        ...
        # ==================== FUSION + DECODER ====================
        # Fuse PredRNN output (64) + future temp features (64) → 128
        fusion_channels = rnn_hidden + hidden_channels[2]
    ...
    def forward(self, context: torch.Tensor, future_temp: torch.Tensor) -> torch.Tensor:
        """
        Forward pass through MicrostructurePredRNN.
        ...
        # ==================== PREDRNN SPATIOTEMPORAL MODELING ====================
        rnn_out = self.pred_rnn(encoded_seq)  # [B, 64, H/8, W/8]
        ...
        fused = torch.cat([rnn_out, fp3], dim=1)  # [B, 128, H/8, W/8]
